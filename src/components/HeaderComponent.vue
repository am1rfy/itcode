  ...
  <nav class="navbar navbar-dark bg-dark">
    <div class="container-fluid">
      ...
      <div class="right-side-header">
        ...
        <button class="btn btn-outline-warning create-btn" type="submit" v-on:click="createNode">Create</button>
      </div>
    </div>
  </nav>
</template>
...
export default {
  ...
  methods: {
    createNode() {
      useLabelStore().items.forEach(item => {
        if (item.title === this.activeLabelName) {
          item.cardsIds.push(useNoteStore().createNote({
            title: '123',
            text: '456'
          }))
        }
