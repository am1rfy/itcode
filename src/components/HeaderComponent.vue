  ...
  <nav class="navbar navbar-dark bg-dark">
    <div class="container-fluid">
      ...
      <div class="right-side-header">
        ...
        <button class="btn btn-outline-warning create-btn" type="submit" @click="createNode" :disabled="activeLabelIsStatic">Create</button>
      </div>
    </div>
  </nav>
</template>
...
export default {
  ...
  computed: {
    labels() {
      return this.labelStore.items
    },
    activeLabelIsStatic() {
      for (let i = 0; i < this.labels.length; i++) {
        if (this.labels[i].title === this.activeLabelName) return false
      }
      return true
    }
  },
  methods: {
    createNode() {
      this.labels.forEach(item => {
        if (item.title === this.activeLabelName) {
          item.cardsIds.push(this.noteStore.createNote({
            title: this.activeLabelName,
            text: '456'
          }))
        }
      })
